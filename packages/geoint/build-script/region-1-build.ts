...
export default async () => {
  ...
  const filteredIso3166Alpha2 = contents.iso3166Alpha2.filter(
    (item: { code: string | null }) => item.code !== null,
  );
  const filteredIata = contents.iata.filter(
    (item: { code: string | null }) => item.code !== null,
  );
  const filteredIcao = contents.icao.filter(
    (item: { code: string | null }) => item.code !== null,
  );
  ...
  const regions = [...filteredIso3166Alpha2, ...filteredIata, ...filteredIcao];
