...
export default async () => {
  ...
  const filePaths = {
    ...
    languageSet: path.join(regionLevel2Path, "pre-translation", language),
  };
  ...
  async function processBatch(batch: { batchText: string; index: number }[]) {
    const results = await Promise.allSettled(
      batch.map(async ({ batchText, index }) => {
        // Skip if file exists
        if (
          fs.existsSync(path.join(filePaths.languageSet, `batch-${index}.txt`))
        ) {
          console.log(
            chalk.yellow(
...
- Please respect the original formatting.`;
  ...
  for (let i = 0; i <= baseSets.length; i++) {
    ...
    if (
      !fs.existsSync(path.join(filePaths.languageSet, `batch-${i + 1}.txt`))
    ) {
